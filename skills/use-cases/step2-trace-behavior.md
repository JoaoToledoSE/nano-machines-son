...
- **Where the flow creates a record, every field the creating code sets that differs from the model/schema default** — and, for each, the code that reads it. A factory/repository quietly overriding a default is a behavior decision: it changes which surfaces the new record appears on, which controls act on it, and what the UI says about it. Enumerating only the fields the creating code *leaves alone* invites the reader to assume the rest are defaults too.
- **What each state/status actually means, treated as a claim to verify** — not settled by a design doc, plan, or decision record. Those record intent at a point in time ("confirmed <date>" included); production can and does diverge. Check the operational record for the flow — the bug/incident tickets on that surface — and if it contradicts or postdates the design claim, the operational reading wins and the doc names its source. Anything on the "could NOT confirm from this repo" list stays unconfirmed until an operational source settles it; an in-repo design doc describing an external system's behavior is not that source.
...
| Documenting only the defaults the creating code leaves alone | Diff what it explicitly sets against the model/schema defaults; every override is a behavior fact — trace it to the surface that reads it |
| A status's meaning taken from a design doc or decision record | That is intent, not behavior. Check the flow's bug/incident history; a production contradiction supersedes it, and a state written by an external system can't be settled from this repo at all |
